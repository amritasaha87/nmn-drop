...
def extract_custom_numbers(s):
    print ('in extract custom numbers')
    ordinal_numbers = []
    for w in s.split(' '):
        if w in words_to_ordinal:
            ordinal_numbers.append(words_to_ordinal[w])
        elif w.endswith('th') or w.endswith('st'):
            try:
                n = w2n.word_to_num(str(token)[:-2])
                ordinal_numbers.append(n)
            except:
                continue       
    frac_numbers = []
    for w in s.split(' '):
